      ...
      <main className="relative flex flex-col justify-start lg:justify-center min-h-screen w-full overflow-x-hidden overflow-y-auto bg-flare-light dark:bg-ambient-dark transition-colors duration-500 px-0 md:px-12 lg:px-24">
        ...
        >
          ...
          >
            ...
            (data?.user as any)?.role === "CUSTOMER" ? (
              <Link
                href="/dashboard"
        ...
        <div className="flex-1 flex flex-col lg:flex-row items-center justify-center w-full max-w-7xl mx-auto z-10 pt-28 lg:pt-20 lg:pb-0 pb-12 gap-8 lg:gap-40">
          ...
          >
            ...
            <motion.div
              variants={itemVariants}
              className="flex lg:hidden justify-center gap-2 mb-8"
            >
              {slides.map((_, index) => (
                <div
                  key={index}
            ...
            <motion.div
              variants={itemVariants}
              className="w-full sm:w-auto flex flex-col items-center gap-4 mt-4 lg:mt-8"
            >
              {status === "authenticated" &&
              (data?.user as any)?.role === "CUSTOMER" ? (
                <Link
                ...
                <>
                  ...
                  <Link
                    href="/login"
                    className="lg:hidden text-sm font-medium text-gray-600 dark:text-gray-400 mt-2"
                  >
                    Already have an account?{" "}
                    <span className="text-brand-cobalt font-bold hover:underline">
                      Sign in
                    </span>
                  </Link>
                </>
            ...
            >
              <div className="flex -space-x-2">
                <div className="w-8 h-8 rounded-full bg-[#e8be66] flex items-center justify-center ring-2 ring-blue-500/20 text-white font-bold text-xs z-10 shadow-sm">
                  A
                </div>
                <div className="w-8 h-8 rounded-full bg-[#82a3f7] flex items-center justify-center ring-2 ring-blue-500/20 text-white font-bold text-xs z-20 shadow-sm">
                  T
                </div>
                <div className="w-8 h-8 rounded-full bg-[#ee9877] flex items-center justify-center ring-2 ring-blue-500/20 text-white font-bold text-xs z-30 shadow-sm">
                  M
                </div>
              </div>
              <div className="text-[#10173b] dark:text-gray-300 text-sm leading-tight">
                <span className="font-bold text-[15px]">4.9</span> rating from{" "}
                <span className="font-bold text-[15px]">500+</span>
                <br />
                customers
              </div>
            </motion.div>
