...
interface OrderWizardClientProps {
  services: AdminService[];
  garments: GarmentItem[];
  userAddress: string;
}

export function OrderWizardClient({ services, garments, userAddress }: OrderWizardClientProps) {
    ...
    if (preselectedService) {
      setOrder((prev) => {
        ...
        return {
          ...
          serviceNames: [
            ...prev.serviceNames, 
            services.find((s) => s.id === preselectedService)?.name || ""
          ].filter(Boolean),
        };
      });
      setStep(2);
  ...
  const goNext = () => {
    ...
    if (step === TOTAL_STEPS) {
      const servicesStr = order.serviceNames.join(", ");
      ...
      if (servicesStr) params.set("services", servicesStr);
  ...
  const steps: Record<number, React.ReactNode> = {
    1: <StepService order={order} onChange={update} onSelectService={handleSelectService} services={services} />,
    2: <StepItems order={order} onChange={update} itemSelectionIndex={itemSelectionIndex} onAddService={handleAddService} onRemoveService={handleRemoveService} garments={garments} services={services} />,
    3: <StepLocation order={order} onChange={update} userAddress={userAddress} />,
