...
import { AdminCustomer, AdminOrder } from "@/data/mock-admin";
...
export function CustomerProfileClient({ customer, customerOrders }: CustomerProfileClientProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-gray-500 mt-1">CL-CUST-{String(customer.customerNumber).padStart(4, '0')}</p>
        </div>
      </div>
            ...
            {activeTab === "loyalty" && (
              <motion.div key="loyalty" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-yellow-50 to-yellow-100/50 dark:from-yellow-500/10 dark:to-yellow-500/5 rounded-3xl border border-yellow-200 dark:border-yellow-500/20 p-6 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-bold text-yellow-600 dark:text-yellow-500 uppercase tracking-wider mb-1">Orders to Next Tier</p>
                    <span className="text-2xl font-black text-yellow-900 dark:text-yellow-400">
                      {customer.ordersToNextTier === 0 ? "Max Tier" : customer.ordersToNextTier}
                    </span>
                  </div>
                </div>
