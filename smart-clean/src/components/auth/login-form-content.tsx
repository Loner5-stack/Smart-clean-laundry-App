...
export function LoginFormContent() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      // Step 1: Validate server-side (rate limiting + explicit DB check)
      const result = await loginAction(formData);

      if (!result.success) {
        const errorMsg = result.error ?? "Invalid email or password.";
        const lowerMsg = errorMsg.toLowerCase();
        if (lowerMsg.includes("email") || lowerMsg.includes("oauth") || lowerMsg.includes("exist")) {
          setErrors({ email: errorMsg });
        } else if (lowerMsg.includes("password") || lowerMsg.includes("incorrect")) {
      ...
      // Step 2: Credentials confirmed valid — call NextAuth signIn.
      // redirect: true (default) lets NextAuth handle the full redirect
      // lifecycle including setting the session cookie properly.
      await signIn("credentials", {
        email,
        password,
        callbackUrl: "/dashboard",
      });
      // Note: execution does not continue past signIn when redirect: true
      // The browser will be navigated to /dashboard (or /onboarding via proxy)
    });
  };
  ...
  return (
    ...
    >
      ...
      <ScrollArea className={`transition-all duration-500 ease-in-out ${scrollClasses}`}>
        <form noValidate onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              <div className={`absolute z-10 inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors ${errors.email ? "text-red-400" : "text-gray-400 group-focus-within:text-brand-cobalt"}`}>
                <Mail size={18} />
              </div>
              <input
                type="email"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <motion.div
              ...
              className="relative group"
            >
              <div className={`absolute z-10 inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors ${errors.password ? "text-red-400" : "text-gray-400 group-focus-within:text-brand-cobalt"}`}>
                <Lock size={18} />
              </div>
              <input
            ...
            <div className="flex justify-end mt-1">
              <span className="text-xs font-bold text-brand-cobalt cursor-not-allowed opacity-60">
