...
exports.riderRoutes = riderRoutes;
const auth_1 = require("../middleware/auth");
/**
 * Rider routes — all require RIDER role.
 * Riders can only see their own assigned orders and update their status.
 */
async function riderRoutes(server) {
    /**
     * GET /api/rider/tasks
     * Returns all orders currently assigned to this rider.
     */
    server.get("/api/rider/tasks", { preHandler: [auth_1.verifyAuth, (0, auth_1.requireRole)("RIDER")] }, async (request, reply) => {
     ...
     */
    server.patch("/api/rider/tasks/:id", { preHandler: [auth_1.verifyAuth, (0, auth_1.requireRole)("RIDER")] }, async (request, reply) => {
        const { id: userId } = request.user;
